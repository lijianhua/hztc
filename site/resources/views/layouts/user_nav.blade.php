<div class="personal-left">
<dl>
    <dt class="personal-left-title"></dt>
    <dd class="personal-left-item"><a href="/users/order" 
      class="{{'我的订单'==Session::get('user_navigator')?'active':''}}">我的订单</a> </dd>
    <dd class="personal-left-item"><a href="/users/info"
      class="{{'广告主信息'==Session::get('user_navigator')?'active':''}}">广告主信息</a> </dd>
    <dd class="personal-left-item"><a href="#">我的收藏</a> </dd>
    <dd class="personal-left-item"><a href="#">我的关注</a> </dd>
    <dd class="personal-left-item"><a href="/users/score"
      class="{{'我的积分'==Session::get('user_navigator')?'active':''}}">我的积分</a> </dd>
    <dd class="personal-left-item"><a href="#">退货清单</a> </dd>
</dl>
</div>
